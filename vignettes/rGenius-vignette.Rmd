...
get_songs(list_song, access_token) %>% 
  select(id, title, artist, album, date, views)
```
...
```
access_token <- "your_token"
search_song("Titanium", access_token, 10, verbose = TRUE) %>% 
  select(id, title, artist, album, date)
```
...
```
access_token <- "your_token"
get_artist(13, access_token) %>% 
  select(id, alternate_name, facebook_name, instagram_name)
```
...
```
access_token <- "your_token"
katty <- get_song_from_artists("katty%20perry", access_token, 20)
...
katty %>% 
  ...
  labs(x ="Albums", y = "Views", title = "Katy Perry Album Results from API") +
  theme_light()
```
